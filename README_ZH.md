...
在另一台机器（NSCC 4×A100-SXM4-40GB，torch 2.4.1+cu121）上对**同一个** λ=0 的 coarse 检查点
（hopper 训练的 `coarse_base_seed0`，直接复用、未重训）做了提取端的完整扫描，只改「高斯 → 网格」
这一步，高斯本身未动，因此 PSNR / SSIM / LPIPS 按构造与 λ=0 组完全相同，只报几何与拓扑指标。

**一行结果**：`q=0` 顶点清洗 + M2-B DBSCAN 剪枝（p95，剪 14.2% 高斯）+ M1+a 背景 Poisson 深度 9，
**碎片分量 2094 → 1194（−43%）、最大分量面数占比 42.3% → 93.9%、边界边 38450 → 11225（−70.8%）、
提取耗时 340 s → 240 s，而 G1 中位距离只变 +0.5%（噪声内）、渲染指标零代价**。
增益可分解为三段且相互独立可叠加：`q=0` 约 −22%、M2-B 剪枝再 −13%、背景 D=9 再 −15%。

| 配置 | D_fg / D_bg | q | 剪枝比例 | 连通分量 ↓ | 碎片 ↓ | 最大分量占比 ↑ | 边界边 ↓ | G1 中位 (abs) ↓ |
|---|---|---|---|---|---|---|---|---|
| 原版基线 | 10 / 10 | 0.1 | — | 2184 | 2094 | 42.3% | 38450 | 0.00405 |
| 仅 q=0 | 10 / 10 | 0 | — | 1651 | 1628 | 93.0% | 11339 | 0.00418 |
| M1+a 背景 D=9 + q=0 | 10 / 9 | 0 | — | 1478 | 1456 | 92.9% | 11033 | 0.00417 |
| M2-B p95 + q=0 | 10 / 10 | 0 | 14.2% | 1434 | 1412 | 94.0% | 11352 | 0.00407 |
| **M2-B + M1+a（最优）** | 10 / 9 | 0 | 14.2% | **1216** | **1194** | **93.9%** | **11225** | 0.00407 |

> 注：该机器自己的原版基线碎片是 2094，hopper 侧同一模型是 2137（差 2%，来自泊松求解与顶点清洗的
> 非确定性）；上表百分比一律以该机器自己的 2094 为基准，不跨机器相减。

**来源声明**：M1（自动 Poisson 深度）逐字移植自 [Anttwo/Frosting](https://github.com/Anttwo/Frosting)
`frosting_extractors/coarse_shell.py` L17–49；M2（DBSCAN 剪枝）思路来自
[prajwalcr/2d-sugar](https://github.com/prajwalcr/2d-sugar) `gaussian_model.py:429`；
本工作的自有改动是 **M1+a**（前景/背景分开估深度）、**M2-B**（把「只留最大簇」改为「保留所有
size ≥ 组内点数一定比例的簇」并分区估计 eps）与 **q / extract_seed 参数化**。

**负结果（如实记录）**：Frosting 原样自动深度算出 D=10，与硬编码相同、零收益；M1+b 用表面采样点估密度
会把前后景深度都顶到上限 10、失去区分度；2d-sugar 原版「只留最大簇」在 Truck 上删掉 80.7% 背景高斯，
是失败案例；「可见表面覆盖率」92.0% → 99.98% 中相当部分来自 Poisson 在天空区域封口的大面片，
不是真实几何，只能作辅助指标。
...
| 内容 | 路径 |
|---|---|
| 指标汇总（9 组）与列说明 | `results_a100/metrics/summary.csv`、`COLUMNS.md` |
| 来源与可比性、逐条命令 | `results_a100/notes/PROVENANCE.md`、`COMMANDS.md` |
| 负结果、完整结果记录、自检 | `results_a100/notes/NEGATIVE_RESULTS.md`、`RESULTS_m_nscc_2026-09-13.md`、`SELF_CHECK.md` |
| 可视化 PNG | `results_a100/figures/`（碎片着色对比、法向/深度 6 配置×4 视角、天空封口证据、扫参图） |
| 代码改动与参数表 | `patches_a100/m1a_q0_extract.patch`、`m2b_cluster_prune.patch`、`PARAMS.md` |
| 复现脚本与顺序说明 | `scripts_a100/README.md`（剪枝 → 提取 → 评测 → 汇总 → 独立复算 → 可视化） |
| 交付清单总览 | `A100_DELIVERY.md` |

完整叙述见技术报告 `outputs/reports/report.pdf` 第 6 章「扩展实验：提取端改进的完整扫描（4×A100）」。
